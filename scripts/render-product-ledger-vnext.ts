...
const pill = (kind: string, value: string) => `<span class="pill ${kind}-${escapeHtml(cls(value))}">${escapeHtml(kind)}: ${escapeHtml(value)}</span>`;
const safeHref = (href: unknown) => typeof href === "string" && /^(?:https?:|#|\.\.?\/|[a-z0-9_-]+\/)/i.test(href);
const link = (value: any) => {
 requireValue(safeHref(value?.href), `unsafe evidence link: ${value?.href}`);
 return `<a href="${escapeHtml(value.href)}" target="_blank" rel="noreferrer">${escapeHtml(value.label)}</a>`;
};
...
export function validateProductLedgerVnext(data: any): void {
 ...
 const canonicalText = data.canonicalSource.html.replace(/<[^>]+>/g, " ").replaceAll("&amp;", "&").replaceAll("&quot;", '"').replaceAll("&#39;", "'").replaceAll("&nbsp;", " ").replace(/\s+/g, " ");
 const truthStates = new Set(["LIVE", "PARTIAL", "BROKEN", "NOT BUILT"]);
 const planStates = new Set(["COMPLETE", "ACTIVE", "NEXT", "BLOCKED", "LATER"]);
 const approvalStates = new Set(["APPROVED", "PROPOSED"]);
 ...
 for (const epic of data.roadmap) {
  ...
  requireValue(truthStates.has(epic.truth) && planStates.has(epic.plan) && approvalStates.has(epic.approval), `${epic.key} has an invalid state`);
  ...
  requireValue(safeHref(epic.gate?.evidence?.href), `${epic.key} gate needs safe evidence`);
  requireValue(!(epic.plan === "COMPLETE" && epic.approval === "APPROVED") || epic.gate.accepted === true, `${epic.key} completed approved gate must be accepted`);
  requireValue(Array.isArray(epic.commitments) && Array.isArray(epic.work), `${epic.key} needs commitment and work arrays`);
  for (const commitment of epic.commitments) {
   requireValue(truthStates.has(commitment.truth), `${commitment.key} has an invalid truth state`);
   requireValue(typeof commitment.eval === "string" && typeof commitment.limit === "string" && canonicalText.includes(commitment.limit) && safeHref(commitment.evidence?.href), `${commitment.key} needs canonical eval, boundary, and evidence`);
   requireValue(!commitment.currentCorrection || data.truthCorrections.some((item: any) => item.text === commitment.currentCorrection), `${commitment.key} correction must come from the truth overlay`);
   requireValue(commitment.evidence.href !== "https://github.com/exocognito/angelmcp/blob/main/docs/product-ledger.html", `${commitment.key} evidence cannot cite the Ledger itself`);
  }
  for (const work of epic.work) {
   requireValue(typeof work.key === "string" && !workKeys.has(work.key), `work key must be unique: ${work.key}`);
   workKeys.add(work.key);
   requireValue(work.kind === "Feature" || work.kind === "Task", `${work.key} must be a Feature or Task`);
   requireValue(truthStates.has(work.truth) && planStates.has(work.plan) && approvalStates.has(work.approval), `${work.key} has an invalid state`);
   ...
   requireValue(Array.isArray(work.mapsTo) && work.mapsTo.length > 0 && work.mapsTo.every((key: string) => epic.mapsTo.includes(key)), `${work.key} mappings must belong to ${epic.key}`);
   requireValue(work.mapsTo.some((key: string) => work.key.startsWith(`${key}.`) || work.key === key), `${work.key} must be namespaced by a canonical mapping`);
   requireValue(typeof work.claim === "string" && work.claim !== work.title && typeof work.blockers === "string" && work.blockers !== "Named by the owning Epic gate.", `${work.key} needs canonical claim and blockers`);
   requireValue(safeHref(work.evidence?.href) && work.evidence.href !== "https://github.com/exocognito/angelmcp/blob/main/docs/product-ledger.html", `${work.key} needs direct evidence`);
  }
 }
...
@media (prefers-reduced-motion: reduce){.vnext-epic summary::before,.vnext-work summary::before,.vnext-commitment summary::before,.vnext-gate summary::before{transition:none!important}}
</style>`;

function renderCommitment(item: any): string {
 const correction = item.currentCorrection ? field("Current correction", escapeHtml(item.currentCorrection)) : "";
 return `<details class="vnext-commitment" data-truth-state="${escapeHtml(item.truth)}"><summary><span class="row-key">${escapeHtml(item.key)}</span><h3>${escapeHtml(item.title)}</h3>${pill("truth",item.truth)}</summary><div class="vnext-detail">${field("Eval",escapeHtml(item.eval))}${field("Limit",escapeHtml(item.limit))}${correction}${field("Evidence",link(item.evidence))}${field("Last verified",escapeHtml(item.verified))}</div></details>`;
}
...
function renderIndex(data: any, canonicalIndex: string): string {
 ...
 const legacyBody = canonicalIndex.replace(/^<section[^>]*>/, "").replace(/<\/section>$/, "").replace(/\sid="index-([^"]+)"/g, ' data-legacy-index-anchor="$1"');
...
export function renderProductLedgerVnext(dataroom: any): string {
 ...
 const replaceRequired = (source: string, search: string, replacement: string, label: string) => {
  if (!source.includes(search)) throw new Error(`canonical ${label} marker not found`);
  return source.replace(search, replacement);
 };
 output = replaceRequired(output, "<title>Angel Product Ledger — approved v0.1</title>", "<title>Angel Product Ledger — proposed vNext</title>", "title");
 output = replaceRequired(output, "</head>", `${styles}</head>`, "head");
 output = replaceRequired(output, "const indexRows=[...document.querySelectorAll('[data-index-key]')];", "const indexRows=[...document.querySelectorAll('#project-index details')];", "Project Index controls");
 output = replaceRequired(output, "<main data-contract-version=", '<main data-proposal-contract-version="proposed-v0.2" data-contract-version=', "contract");
 ...
 const snapshot = JSON.stringify(dataroom, null, 2).replace(/<\/script/gi, (match) => `<\\/${match.slice(2)}`);

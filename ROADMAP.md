...
- Those decisions are assembled into one buildable target-state document in
  [docs/aprd](docs/aprd/README.md) — the goals map, demonstrable commitments,
  station detail, system boundary, and the words the product is described in.
  It reads the ADRs and product decisions; it does not replace them, and where
  it disagrees with a decision record on a settled point, fix the APRD.
...
| ✅ **Address Angels as `@account/angel`, with production as the default.** Tracked in [#3](https://github.com/exocognito/angel-cloud/issues/3); decided in [PD 0001](docs/product-decisions/0001-angel-coordinate-scheme.md) and [PD 0003](docs/product-decisions/0003-preview-is-opt-in.md) as amended by [PD 0005](docs/product-decisions/0005-preview-binds-its-own-connections.md). The second environment is named `preview`, the server deploys a published Version straight to production in one step, and the legacy route and management dialect keep answering the pinned CLI. The pinned `@smcllns/angel-core` 0.3.0 makes bare `angel publish` deploy to production and reserves `--preview` for the opt-in path. | An Angel answers MCP at `/@handle/angel` with no environment in the URL; `angel publish` on a fresh Angel produces a live, callable Angel in one command; the old route still answers through the cutover. |

...
| ⬜ **Dogfood the first Angel end-to-end through the publishing API.** Tracked in [#5](https://github.com/exocognito/angel-cloud/issues/5); the run sheet is [docs/dogfooding-guide.html](docs/dogfooding-guide.html), a standing guide plus a dated session sheet that is replaced wholesale each run. | Starting from an empty directory with no repo clone, Sam and an agent create, publish, promote, and query a new Angel over MCP; every friction found becomes an issue. |

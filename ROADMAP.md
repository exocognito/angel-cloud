...
- Between M1 and M2, any order:
  ...
  - ✅ address Angels as `@account/angel`, production by default
    ([#3](https://github.com/exocognito/angel-cloud/issues/3))
...
| ⬜ **Dogfood the first Angel end-to-end through the publishing API.** Tracked in [#5](https://github.com/exocognito/angel-cloud/issues/5); the run sheet is [docs/dogfooding-guide.html](docs/dogfooding-guide.html), a standing guide plus a dated session sheet that is replaced wholesale each run. Findings from each run are kept in [docs/dogfooding](docs/dogfooding/); the first is [2026-07-29](docs/dogfooding/2026-07-29-m2-findings.md). | Starting from an empty directory with no repo clone, Sam and an agent create, publish, promote, and query a new Angel over MCP; every friction found becomes an issue. |
| ✅ **Public agent docs: docs index, `llms.txt`, `SKILL.md`** so agents use the API, not the repo. Tracked in [#4](https://github.com/exocognito/angel-cloud/issues/4). The canonical host is `docs.angelmcp.ai`; the apex `/docs` and `/llms.txt` paths redirect to it ([docs/domain-architecture.md](docs/domain-architecture.md)). Until the zone moves ([#6](https://github.com/exocognito/angel-cloud/issues/6)), the site serves from the stable interim URL `https://angelmcp-docs-demo.sam-633.workers.dev`. | An agent given only the docs URL — no repo access — can walk a user through the whole first-Angel journey to a promoted Angel answering MCP calls. |
...
| ✅ **Address Angels as `@account/angel`, with production as the default.** Tracked in [#3](https://github.com/exocognito/angel-cloud/issues/3); decided in [PD 0001](docs/product-decisions/0001-angel-coordinate-scheme.md) and [PD 0003](docs/product-decisions/0003-preview-is-opt-in.md) as amended by [PD 0005](docs/product-decisions/0005-preview-binds-its-own-connections.md). The second environment is renamed `preview`, the server deploys a published Version straight to production in one step, and the legacy route and management dialect keep answering the pinned CLI. The CLI-side default flip shipped in `@smcllns/angel-core@0.3.0`: bare `angel publish` goes to production and `--preview` is the opt-in. | The MCP endpoint at `/@handle/angel` answers with no environment in the URL; `angel publish` on a fresh Angel produces a live, callable Angel in one command; the old route still answers through the cutover. |

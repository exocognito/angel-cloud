...
function checkedArtifact(slug: string): ManagementVersionArtifact {
  const canonicalSource = readFileSync(new URL(`../../examples/angels/${slug}/build/angel.version.json`, import.meta.url), "utf8").trim();
  const digest = readFileSync(new URL(`../../examples/angels/${slug}/build/angel.version.sha256`, import.meta.url), "utf8").trim();

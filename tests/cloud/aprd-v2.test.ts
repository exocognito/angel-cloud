...
describe("APRD v2", () => {
  ...
  test("marks the APRD draft unapproved and gives the Product Ledger precedence", () => {
    ...
    expect(roadmap).toContain("published it on 2026-08-03 as");
    expect(roadmap).not.toContain("That package is unbuilt");
    expect(roadmap).not.toContain("is unpublished, so the command");
  ...
  test("makes the CLI-first golden path an executable v2.1 contract", () => {
    ...
    expect(section).toContain("v2.1 install path: <strong>published</strong>");
    expect(section).toContain("It published the same day through npm trusted publishing with provenance");
